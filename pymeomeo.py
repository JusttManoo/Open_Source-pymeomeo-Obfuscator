...
def PhTanSang1907():
    globals()["_"]=[[[[[(('PhTanSang') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(__import__('sys')) != "{eval("__import__('sys')")}":[[[[[(('PhTanSang') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(__import__('sys').exit) != "{eval("__import__('sys').exit")}":[[[[[(('PhTanSang') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(__import__('os')) != "{eval("__import__('os')")}":[[[[[(('PhTanSang') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(__import__('os').system) != "{eval("__import__('os').system")}":[[[[[(('PhTanSang') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(type(open)) != "<class 'builtin_function_or_method'>":[[[[[(('PhTanSang') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if str(eval("open")) != '<built-in function open>': [[[[[(('PhTanSang') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
...
try:
    ...
    if _author != ('PhTanSang', 'https://www.facebook.com/sang.phung.85570/'):
        raise
    if ';' in j[0]:
        raise
    if ';' in j[1]:
    ...
    if ';' in j[2]:
        raise
    if j[4][0:196] != "try:_PhTanSang['exec'](_PhTanSang['__import__']('marshal').loads(_PhTanSang['__import__']('zlib').decompress(_PhTanSang['__import__']('bz2').decompress(_PhTanSang['__import__']('base64').a85decode":
        raise
    if str(j[4])[-2:] != ")\n":
        raise
    if str(j[5])[-1] == ';':
        raise
    if _PhTanSang_version != ('PREMIUM','User : {nhapuser}','main'):
        raise
    if _obf != ('pymeomeoV2.1')[(lambda : 0)()]:
        raise 
    if "('pymeomeoV2.1')[(lambda : 0)()]" not in j[0]:
        raise
...
if _author != ('PhTanSang', 'https://www.facebook.com/sang.phung.85570/'):
    globals()["_"]=[[[[[(('PhTanSang') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
if _PhTanSang != vars(globals()['__builtins__']):
    globals()["_"]=[[[[[(('PhTanSang') * 987654321)] * 987654321] * 987654321] * 987654321] * 987654321] * 2123000000 * 2123000000
"""
code = antica2+code
...
class PYMEOMEOMEWWW(list):
    ...
    def format_PhTanSang(pymeomeo_, PhTanSang):
        row = []
        row.append('bo la nguoi dep trai')
        if PhTanSang.line:
            stripped_line = PhTanSang.line.strip()
            row.append('cuts di')

            line = PhTanSang.pmeooowwwww
            orig_line_len = len(line)
            frame_line_len = len(PhTanSang.line.lstrip())
            stripped_characters = orig_line_len - frame_line_len
            if (
                PhTanSang.dethuongvacute is not None
                and PhTanSang.meomeocute is not None
            ):
                start_offset = _byte_offset_to_character_offset(
                    line, PhTanSang.dethuongvacute)
                _0xFFFFFFFFF = _byte_offset_to_character_offset(
                    line, PhTanSang.meomeocute)
                meopymeomeo = line[start_offset:_0xFFFFFFFFF]
                anchors = None
                if PhTanSang.lineno == PhTanSang.meomeodethuong:
        ...
        if PhTanSang.locals:
            for name, pymewwww in sorted(PhTanSang.locals.items()):
                row.append('aa')
        return ''.join(row)
    ...
    def format(pymeomeo_):
        ...
        ln = None
        count = 0
        for PhTanSang in pymeomeo_:
            formatted_frame = pymeomeo_.format_PhTanSang(PhTanSang)
            if formatted_frame is None:
                continue
            if (lf is None or lf != PhTanSang.filename or
                ll is None or ll != PhTanSang.lineno or
                ln is None or ln != PhTanSang.name):
                if count > _RECURSIVE_CUTOFF:
                    count -= _RECURSIVE_CUTOFF
                    result.append('')
                lf = PhTanSang.filename
                ll = PhTanSang.lineno
                ln = PhTanSang.name
                count = 0
            count += 1
...
if str(__import__('sys').version[0:4]) != '{str(eval("__import__('sys').version[0:4]"))}':
    ...
    print(">> Loading..",end='\\r')
    _PhTanSang['exec'](_PhTanSang['__import__']("marshal").loads(_PhTanSang['__import__']("zlib").decompress(_PhTanSang['__import__']('bz2').decompress(_PhTanSang['__import__']('base64').a85decode({code})))))
"""
    code = marshal.dumps(code)
    code = zlib.compress(code)
    code = bz2.compress(code)
    code = base64.a85encode(code)
    code = f"""_obf = ('pymeomeoV2.1')[(lambda : 0)()]
_author = ('PhTanSang', 'https://www.facebook.com/sang.phung.85570/')
_PhTanSang_version = ('PREMIUM','User : {nhapuser}','{vaidai}')
_PhTanSang = vars(globals()['__builtins__'])
try:_PhTanSang['exec'](_PhTanSang['__import__']('marshal').loads(_PhTanSang['__import__']('zlib').decompress(_PhTanSang['__import__']('bz2').decompress(_PhTanSang['__import__']('base64').a85decode({code})))))
